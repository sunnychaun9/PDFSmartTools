...
export class ProgressTracker {
  ...
  private lastEmittedProgress: EnhancedProgress | null = null;

  /**
   * @param totalItems Total number of items to process
   * @param minUpdateInterval Minimum ms between updates (default 100ms, use 250ms for long operations)
   */
  constructor(totalItems: number, minUpdateInterval: number = 100) {
   ...
   * Returns null if throttled (update interval not reached and progress unchanged).
   */
  update(currentItem: number, status: string): EnhancedProgress {
    ...
    this.lastEmittedProgress = result;
    return result;
  }
   ...
   */
  reset(totalItems: number, minUpdateInterval?: number): void {
    this.startTime = Date.now();
    this.lastUpdateTime = this.startTime;
    this.totalItems = totalItems;
    this.currentItem = 0;
    ...
    if (minUpdateInterval !== undefined) {
      this.minUpdateInterval = minUpdateInterval;
    }
  }

  /**
   * Check if enough time has passed since the last update to warrant a new state update.
   * Use this in callbacks to skip redundant setState calls.
   */
  shouldUpdate(): boolean {
    return (Date.now() - this.lastUpdateTime) >= this.minUpdateInterval;
  }
