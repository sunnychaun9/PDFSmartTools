...
import { pickPdfFile } from '../../../native/filePicker';
...
function HomeScreen() {
  ...
  const cardWidth = useMemo(() => (width - spacing.lg * 2 - CARD_GAP) / 2, [width]);
        ...
        <View style={styles.headerTop}>
          ...
          <Animated.View style={{ transform: [{ scale: themeToggleScale }] }}>
            <TouchableOpacity
              ...
              accessibilityLabel={`Dark mode, ${isDark ? 'on' : 'off'}`}
          ...
          >
            ...
            >
              <Icon
                name="grid"
                size={16}
                color={viewMode === 'grid' ? colors.textOnPrimary : theme.textTertiary}
              />
            </View>
            <View
              style={[
            ...
            >
              <Icon
                name="list"
                size={16}
                color={viewMode === 'list' ? colors.textOnPrimary : theme.textTertiary}
              />
            </View>
          </Pressable>
        </Animated.View>
  ...
  ), [fadeAnim, slideAnim, isDark, theme, themeToggleScale, handleThemeToggle, themeToggleAnim, toggleScale, toggleViewMode, viewMode, isPro]);

  const renderGridItem = useCallback(({ item, index }: { item: Tool; index: number }) => (
    <Animated.View
      style={[
        styles.cardWrapper,
        { width: cardWidth },
  ...
  ), [cardWidth, fadeAnim, slideAnim, handleToolPress]);

  const renderListItem = useCallback(({ item }: { item: Tool }) => (
    <Animated.View
      style={{
        opacity: fadeAnim,
        transform: [{ translateY: slideAnim }],
  ...
  ), [fadeAnim, slideAnim, handleToolPress]);
