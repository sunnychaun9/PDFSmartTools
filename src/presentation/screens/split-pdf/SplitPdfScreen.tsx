...
import {
  ...
  SplitResult,
  SplitOutputFile,
} from '../../../native/pdfSplitter';
import { sharePdfFile } from '../../../native/shareService';
import { loadInterstitialAd, showInterstitialAd } from '../../../domain/ads/adService';
import { getRemaining, FEATURES } from '../../../domain/featureGating/usageLimitService';
...
export default function SplitPdfScreen() {
  ...
  // Operation engine
  const { isRunning, progress, execute, updateProgress } = useOperation<SplitResult>({
    tag: 'SplitPdf',
    featureKey: FEATURES.PDF_SPLIT,
    timeoutMs: 300000,
  });
  ...
  const handleSplit = useCallback(async () => {
    ...
    const result = await execute(async (_token) => {
      const baseName = selectedFile.name.replace('.pdf', '');

      return await splitPdf(selectedFile.localPath, baseName, {
        ranges,
        isPro,
        onProgress: (progressInfo) => {
          const currentRange = Math.max(1, Math.ceil((progressInfo.progress / 100) * ranges.length));
          updateProgress(currentRange, `Splitting range ${currentRange} of ${ranges.length}...`);
        },
      });
    });
    ...
    if (result.status === 'success') {
      ...
      setErrorModal({ visible: true, title: 'Split Failed', message: result.error! });
    }
  }, [selectedFile, splitMode, rangeInput, selectedPages, pageCount, isPro, refreshRemainingUses, canProceedWithFeature, consumeFeatureUse, onSuccessfulAction, execute, updateProgress]);
  ...
  return (
    <SafeScreen>
      ...
      >
        ...
        <Button
          title={isRunning ? 'Splitting...' : 'Split PDF'}
          onPress={handleSplit}
          loading={isRunning}
          disabled={isRunning}
          fullWidth
