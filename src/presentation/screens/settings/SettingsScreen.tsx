...
import { shareText } from '../../../native/shareService';
import {
  getAppSettings,
  saveAppSettings,
  getCompressionLabel,
  ...
  savePrivacySettings,
  PrivacySettings,
} from '../../../data/storage/pdfStorage';
import { setCrashReportingEnabled } from '../../../infrastructure/crashlytics';
import { setAnalyticsEnabled } from '../../../infrastructure/analytics';
import { setPerformanceEnabled } from '../../../infrastructure/performance';
...
function SettingsScreen() {
  ...
  // Debug: 5-tap on Version to open stress test screen
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      setPrivacySettingsState(privacy);
    };
    loadSettings();
  }, []);
  ...
  const handleToggleCrashReporting = useCallback(async (value: boolean) => {
    setPrivacySettingsState(prev => ({ ...prev, crashReportingEnabled: value }));
    await savePrivacySettings({ crashReportingEnabled: value });
    await setCrashReportingEnabled(value);
  }, []);

  const handleToggleAnalytics = useCallback(async (value: boolean) => {
    setPrivacySettingsState(prev => ({ ...prev, analyticsEnabled: value }));
    await savePrivacySettings({ analyticsEnabled: value });
    await setAnalyticsEnabled(value);
  }, []);

  const handleTogglePerformance = useCallback(async (value: boolean) => {
    setPrivacySettingsState(prev => ({ ...prev, performanceEnabled: value }));
    await savePrivacySettings({ performanceEnabled: value });
    await setPerformanceEnabled(value);
  }, []);
        ...
        >
          ...
          <View style={[styles.card, { backgroundColor: theme.surface }]}>
            <SettingItem
              ...
              title="Crash Reports"
              subtitle="Help us fix bugs by sending crash data"
              isDark={isDark}
              theme={theme}
              showChevron={false}
              rightElement={
                <Switch
                  ...
                  trackColor={{ false: theme.surfaceVariant, true: `${colors.primary}60` }}
                  thumbColor={privacySettings.crashReportingEnabled ? colors.primary : theme.textTertiary}
                />
              }
            />
            <View style={[styles.divider, { backgroundColor: theme.divider }]} />
            <SettingItem
              ...
              rightElement={
                <Switch
                  ...
                  trackColor={{ false: theme.surfaceVariant, true: `${colors.primary}60` }}
                  thumbColor={privacySettings.analyticsEnabled ? colors.primary : theme.textTertiary}
                />
              }
            />
            <View style={[styles.divider, { backgroundColor: theme.divider }]} />
            <SettingItem
              ...
              subtitle="Help us optimize app speed"
              isDark={isDark}
              theme={theme}
              showChevron={false}
              rightElement={
                <Switch
                  value={privacySettings.performanceEnabled}
                  onValueChange={handleTogglePerformance}
                  trackColor={{ false: theme.surfaceVariant, true: `${colors.primary}60` }}
                  thumbColor={privacySettings.performanceEnabled ? colors.primary : theme.textTertiary}
                />
              }
            />
          </View>
